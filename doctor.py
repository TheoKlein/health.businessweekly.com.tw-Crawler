...
def getDoctor(page):
    ...
    for result in soup.findAll("article", {"class":"searchresults"}):
        ...
        part = left_info.findAll("li")[0].text.replace("科別：", "")
        tel = left_info.findAll("li")[1].text.replace("電話：", "")
        address = left_info.findAll("li")[2].text.replace("地址：", "")
